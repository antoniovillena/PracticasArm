...
con diferentes capas. Cada modo tiene sus propios registros {\tt sp}, {\tt lr} y {\tt spsr} de tal
...
asociado, que contiene la instrucción de salto a la rutina que debe
...
se va a producir una interrupción que viene de una única fuente, por
lo que en la RTI básicamente lo que haremos es encender el LED.
...
\begin{lstlisting}[caption={inter2.c},label={lst:codigoPract5_2}]
        .include  "inter.inc"
.text
        mov     r0, #0        @apunto tabla excepciones
        ADDEXC  0x18, irq_handler
...
\begin{lstlisting}[caption={inter3.c},label={lst:codigoPract5_3}]
        .include  "inter.inc"
.text
        mov     r0, #0        @apunto tabla excepciones
        ADDEXC  0x18, irq_handler
...
es leer todas las entradas a la vez períodicamente
...
\begin{lstlisting}[caption={Programa principal de inter5.c},label={lst:codigoPract5_5}]
        mov     r0, #0        @apunto tabla excepciones
        ADDEXC  0x18, irq_handler
        ADDEXC  0x1c, fiq_handler
        mov     r0, #0xd1     @modo FIQ, FIQ&IRQ desact
